...
<body style="width:2000px; margin: auto; margin-top: 50px;">
    ...
    <div class="row">
        ...
        <div class="col-sm-6">
            ...
            ${paging}
            <ol id="paging" style="display: flex;">
					
				<!-- [이전으로]에 대한 사용여부 처리 -->
				<c:if test="${paging.beginPage == 1 }"> 
					<c:if test="${paging.nowPage > paging.numPerBlock }">
						<li class="disable"> &lt; 이전</li>
					</c:if>
				</c:if>
				<c:if test="${paging.beginPage != 1 }">
					<li>
						<a href="/CYWORLD/guestBook?userCode=${userCode }&visitHost=${visitHost }&type=getList&cPage=${paging.beginPage - 1 }"> &lt;이전 </a>
					</li>
				</c:if>
				
				<!-- 블록내에 표시할 페이지 태그 작성(시작페이지~끝페이지) -->
				
				<c:set var="end" value="${paging.endPage }"/>
				<c:if test="${(paging.totalRecord % paging.numPerPage == 0) && (paging.totalRecord < paging.numPerPage * paging.numPerBlock) }"> 
					<c:set var="end" value="${paging.endPage-1 }"/>
				</c:if>
				<c:forEach var="pageNo" begin="${paging.beginPage }" end="${end }">
